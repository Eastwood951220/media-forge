...
def test_backup_files_list_api_lists_empty_directory(client, auth_headers, tmp_path: Path, monkeypatch):
    config_file = tmp_path / "backup.conf"
    monkeypatch.setattr(
        "backend.app.modules.backup.router.get_backup_config_service",
        lambda: BackupConfigService(config_file=config_file, project_root=tmp_path),
    ...
    response = client.get("/api/backup/files", headers=auth_headers)
