...
export default function UrlEntryFields({
  index,
  onNameExtracted,
  onUrlTypeDetected,
}: UrlEntryFieldsProps) {
  ...
  useEffect(() => {
    ...
    if (detected && detected !== currentType) onUrlTypeDetected(index, detected)
    ...
    <>
      <Form.Item noStyle shouldUpdate={(prev, cur) => prev.urls?.[index]?.url !== cur.urls?.[index]?.url}>
        {({ getFieldValue }) => {
          ...
          const detected = currentUrl ? detectUrlType(currentUrl) : null
          ...
          return (
            <Button
              ...
              onClick={async () => {
                ...
                setExtracting(true)
                try {
                  const result = await extractTaskName(currentUrl, detected ?? 'detail')
